...
- 当前源码版本为 `0.4.20`。MCP 顶层固定 18 个工具，其中新增只读 `audit_log` 查询当前 Owner 的 Hub 本地操作审计，继续使用 `fsprobe` 按需恢复长会话工具物化；Direct API 仍保持原 17 工具权限面，不开放审计读取。本版在 0.4.19 的能力地图与发布安全基础上，加入直接访问密钥能力，并收敛 Windows Node 的隐藏启动、开机自启动、桌面快捷方式和默认窗口行为。OAuth/PKCE 与 Node/WSS 协议不变。
