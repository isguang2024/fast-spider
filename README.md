...
- `ai_control` 1.2 已是多 AI Harness 控制面：当前支持 `codex` 与 `claude_code`；`provider.readiness` 分开报告 route/provider/harness/session backend/create readiness，`session.create` 支持 `visible|internal` 双模式、独立 backend/visibilityTarget 与持久幂等记录。Codex `backend=chatgpt_cloud` 用 Codex app-server 的 ChatGPT 登录态 + 自解 Sentinel 走官方 `/backend-api/f/conversation` 创建云端会话，`externalIdType=chatgpt_conversation`，会话出现在账号的 ChatGPT 聊天列表；依赖本机 Codex app-server 已登录。
...
```bash
# 启动 Hub
# 首次启动必须显式注入一次性管理员密码（请替换为你生成的强密码）；
# 后续启动可省略该变量。不要把密码写入仓库或普通日志。
FAST_SPIDER_ADMIN_PASSWORD='<your-unique-admin-password>' go run ./cmd/hub --data-dir ./data
...
# 用户名：admin，密码为首次启动时显式注入的值；没有公开默认密码。
