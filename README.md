...
- `ai_control` 1.2 已是多 AI Harness 控制面：当前支持 `codex` 与 `claude_code`；`provider.readiness` 分开报告 route/provider/harness/session backend/create readiness，`session.create` 支持 `visible|internal` 双模式、独立 backend/visibilityTarget 与持久幂等记录。ChatGPT cloud conversation 创建保持明确 unsupported，不接入插件私有接口。
