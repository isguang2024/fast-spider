...
`FastSpider_Local` exposes three current-user tools: `local_machine`, `local_capability`, and `collaboration_control`. The collaboration tool is a thin Local Bridge adapter; claim, frozen-packet digest, receipt, confirmed no-create rejection, uncertainty, recovery, and bounded token storage run inside the existing Node process in Go. It does not start Python or a helper daemon, and the capability is deliberately absent from the Hub-routable Node catalog. The public `FastSpider_FS` remains the only Cloud dispatch transport.
