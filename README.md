...
`FastSpider_Local` exposes three current-user tools: `local_machine`, `local_capability`, and `collaboration_control`. The collaboration tool is a thin Local Bridge adapter implemented inside the existing Node process. It can atomically claim a frozen SQLite READY round, create or reuse its ChatGPT Cloud CHAT through the co-located AgentController, persist same-key recovery phases, and deliver the result through a local callback claim/ack queue. It does not start Python or a helper daemon, does not route Cloud create/send through the Hub, and remains absent from the Hub-routable Node catalog. The public `FastSpider_FS` collaboration contract remains available as the remote compatibility path.
