...
The Windows Node UI asks for a Codex session mode on first launch and stores that choice in its local configuration. **Shared mode** is the default recommendation: Fast Spider does not claim loaded sessions through Codex Desktop IPC, so Desktop can open them without showing “already open in another application.” **FS managed mode** enables the owner/control bridge for FS-loaded local sessions. The Node UI setting is authoritative; `FAST_SPIDER_CODEX_DESKTOP_BRIDGE` remains a compatibility fallback for headless `run`/automation processes that do not use the Node UI. For a registered local Desktop thread, `ai_control session.send` and callback wakeups now discover the Desktop owner, read its current stream snapshot, and start the Turn through Desktop IPC only when `threadRuntimeStatus=idle`; a busy owner returns `AGENT_SESSION_BUSY` so callback delivery remains queued instead of entering the active Turn. Only an unavailable or incompatible owner falls back to Fast Spider's managed app-server. Public `ai_control` discovery and local session results include `desktopBridge` state. Continuous native Desktop conversation snapshot/patch streaming remains unsupported.
