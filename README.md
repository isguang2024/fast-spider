...
- remote creation of a local Codex task: `ai_control session.create` with `providerId=codex`, `backend=codex_local`, absolute `workingDirectory`, and a stable `idempotencyKey`
...
Local Codex has one execution path: the Node owns one long-running `codex app-server --stdio` and uses `thread/list`, `thread/read`, `thread/start`, `thread/resume`, and `turn/start` for both local and Hub-forwarded calls. Results report only `executionMode=codex_app_server` and `owner=fast_spider_node`; sends and callback nudges are successful only when Codex returns a non-empty `turnId`. The Codex Desktop state file is read only for `projects.list` metadata and is never used to authorize, create, send, synchronize, or mutate tasks.

`FastSpider_Local` exposes three current-user tools: `local_machine`, `local_capability`, and `collaboration_control`. The collaboration tool is a thin Local Bridge adapter implemented inside the existing Node process. Its structured SQLite lifecycle atomically claims frozen READY rounds, enforces project and write-scope boundaries, creates or reuses ChatGPT Cloud CHATs through the co-located AgentController, persists same-key recovery phases, and delivers results through a local callback claim/ack queue. It uses no Python or JSON intermediary state, revalidates `local_file` metadata at claim and acknowledgement, and leaves invalid batches pending instead of retiring their routes. It does not route Cloud create/send through the Hub and remains absent from the Hub-routable Node catalog. The public `FastSpider_FS` collaboration contract remains available as the remote compatibility path.
