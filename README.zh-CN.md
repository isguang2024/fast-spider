...
Fast Spider 以 Codex 作为主要工程协作者。维护者负责产品方向、架构决策、验收
标准、安全与隐私边界以及最终发布决策；Codex 在维护者监督下参与代码实现、
测试、文档、代码审查、问题定位和持续迭代。

这种协作方式已经融入项目的日常工程流程：

- 把维护者提出的需求转化为边界明确的实现方案；
- 在 Windows、Linux 和 macOS 上实现并验证跨平台改动；
- 审查代码、失败路径、权限和隐私边界；
- 维护文档、发布门禁和可复现的验证证据。

## 三部分产品结构

Fast Spider 由三个职责清晰的运行部分组成：

| 部分 | 职责 |
|---|---|
| Web Console | 面向 owner 和运维人员的网页管理后台。它通过 Hub 管理 Node、凭据、任务、路由和审计记录，本身不直接执行本机操作。 |
| Hub | 后端服务与控制平面，负责身份认证、API、路由、任务生命周期、策略协调和审计存储。 |
| Node | 安装在每台 Windows、Linux 或 macOS 电脑上的客户端。它主动连接 Hub，并以本机操作系统用户权限实际执行文件、Shell、Git、浏览器和 AI 会话操作。 |
...
推荐在 Codex 中打开本仓库，让 Codex 根据目标环境部署 Web Console、Hub 和
Node。Codex 可以检查操作系统环境、遵循项目记录的安全边界、执行必要检查，
并报告最终服务状态。

可以直接向 Codex 提交下面的请求：

```text
请从当前仓库部署 Fast Spider。先说明准备采用的 Web Console、Hub 和 Node
拓扑；除非我明确批准，否则 Hub 只在本地开放，不使用公网 Tunnel。凭据必须在
本机生成，不得提交到仓库或暴露在日志中。完成文档要求的检查，并报告最终访问
地址和服务状态。
```
