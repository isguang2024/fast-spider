...
- stdout/stderr 在线事件只保留有界窗口；Node 同时保存每 Job 最多 10 MiB 的完整本地日志，终态后默认保留 24 小时，并由每小时维护任务清理。
- 需要长期保留时通过 `artifact_get uploadJobLog` 显式转 Artifact；Artifact 默认 30 天。
- 日志按大小/时间限制，删除有上限，不依赖“下一次任务启动”才触发清理。
...
当前 Artifact 硬上限：单文件 100 MiB、单机未过期总量 512 MiB、Owner 未过期总量 2 GiB、单机同时上传 4 个；达到上限直接拒绝新上传，不靠磁盘写满后补救。

删除顺序：失败/过期临时上传 → 已过期普通 Artifact → 24 小时以上 Job 本地日志 → 按策略过期数据。不能为了腾空间删除未过期安全审计或数据库备份而不告警。
