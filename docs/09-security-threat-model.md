...
- **场景**：后台创建的连接令牌被截屏、Shell 历史、剪贴板或旁观者窃取。
- **风险**：High。
- **控制**：随机高熵、数据库仅保存哈希、明文只展示一次、可设置有效期、可立即撤销；令牌只允许机器登记接口，不能访问 MCP/管理 REST；Node 本地 UI 只在内存中使用输入值，登记后不持久化令牌。
- **剩余风险**：连接令牌设计为可复用，泄露者在撤销/过期前可能登记一台或多台新设备，因此后台必须让新增设备清晰可见并支持立即撤销。对敏感环境可使用短有效期或登记完成后主动撤销令牌。
...
### T22b Node 本地控制 UI 被网页或其他本机进程调用

- **场景**：恶意网页尝试从浏览器跨 Origin 调用 `127.0.0.1` 控制面，或同机其他进程探测本地端口。
- **风险**：Medium/High。
- **控制**：只绑定 `127.0.0.1`；不承载 MCP/Capability；写 API 要求进程启动时随机生成且仅嵌入本地页面的 UI secret，并校验 Origin；页面使用 CSP/no-store。关闭 UI 窗口不会开放额外远程入口。
- **剩余风险**：当前个人模式把同一 OS 用户视为本机信任边界，已完全控制该用户会话的恶意进程仍可能读取页面/进程上下文；不为此增加第二套 Local Client 身份系统。
