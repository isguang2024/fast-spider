...
- MCP 固定 17 个工具，不提供目录列表工具；`thinking_team` 只提供调用侧角色/部门/流程/资料室协议且不调用本机 AI Provider；`working_context` 在同一工具内提供 Plan/Task + Markdown Task Workspace，不演化成长期 AI Memory。
