...
0.4.105 区分创建前校验失败和请求发出后的不确定结果：Sentinel 准备失败时尚未发送会话请求，释放该次预约，后续仍用原幂等键正常重试；请求已发出且结果不明时继续保留预约并核对，不能直接重建。瞬时校验失败采用有界退避并遵守 Retry-After。界面将“派发确认等待”与执行中、验收中分开。

## 在线会话收尾（0.4.106）

`runner.archive` 仍只归档本地任务记录。在线 CHAT 归档由 Node 自动处理：当前块已验收（隔离工作区须完成集成）或已取消，正式终态结果已落盘且回调已 ACK 后，低优先级队列调用在线 `is_archived=true`。历史代有已确认终态结果且不再复用时同样收尾；结果或回调证据不足的会话保留。不会自动删除在线会话。

关联 Job 只查询本地状态，未结束时推迟归档，不发送取消。换代保存关联 Job ID。正常完成使用已有终态结果与 ACK，不为归档反复下载会话全文。业务执行槽与订阅仍由已有结果/ACK 流程释放，归档不占业务槽。

任务返回独立的 `sessionCloseout` 数组，包含 sessionId、round、state、nextAt、lastError、archivedAt。状态不进入 reviewToken，不触发业务重验。全局最多一个异步归档请求，完成后至少间隔 30 秒；失败指数退避并尊重 Retry-After，429 的收尾冷却跨重启保留。收尾失败不阻断业务派发，不把业务已完成改成失败。队列恢复沿用同一 sessionId，在线归档是幂等操作。

正在复用或准备复用的会话不归档；已归档会话后续需复用时先取消在线归档。收尾中的原块等待这一个有界请求结束后再续发，其他块继续运行。页面把在线收尾与业务状态分开展示。
