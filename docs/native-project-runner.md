...
- `after` 等待整块验收；`requires:[{"taskId":"上游 ID 或新块 key","key":"api-contract","version":"v1"}]` 只等待准确版本的阶段快照。生产者通过 `runner.checkpoint` 的 `outputs:[{"key":"api-contract","version":"v1","summary":"稳定接口","path":"项目内接口文件"}]` 发布。单文件上限 64 KiB，Node 保存不可变副本和摘要；提交阶段产出不完成生产者，不代表代码已集成。最终集成块保留真实验收依赖，消费者读取包内 `requiredArtifacts`。
- 云端规划可给完整任务块或未派发的 `revise` 指定 `workspace:{"mode":"worktree"}`。默认 shared；只读、非冲突、小任务不创建 worktree。目录在派发时异步创建，同块修复复用原目录；现有在途任务不迁移。分支基于已提交的主仓基线，不包含主仓未提交改动，规划不得假设这些改动已存在于分支。
- 分支任务只在分配工作区的授权写域开发和提交。Node 在报告与本块检查获得云端采纳后进入 `awaiting_integration`，该等待不占云端槽。集成队列按仓库串行，先等待该仓库共享 writer 结束；其他 worktree、其他仓库继续执行。合入只允许安全 fast-forward，分叉、脏文件冲突和检查失败交原块修复，禁止强制覆盖或自动 stash。
- 合入后在集成根运行受影响的已配置检查，通过后自动记为 accepted；失败交回云端。已合入且无未保存/独有提交的任务目录才安全回收。取消后仍有未合入成果的目录保留，不能因取消而丢失工作。Tibbs 使用 `V:/temp/Tibbs/worktrees/` 和现有共享依赖入口。
- `queueReason` 在任务列表、详情及 `runner.context` 中返回结构化原因：硬依赖、阶段产出、写域、全局或任务区容量、退避、准备和集成。空闲槽与阻塞队列并存时触发基于当前事实的一次云端复核，未变化的计划依据不会反复创建规划。

## 验收状态与并发结果（0.4.104）

任务的 `presentation` 提供实际阶段、原因、负责验收的任务/会话和下一处理时间。待验收表示尚无正在执行的已分配验收任务；验收中依据活跃 planner 的明确 `reviewTargets` 和对应结果快照，不能从普通 `watching` 推断。报告缺失、等待检查、检查执行中、检查失败、验收结果应用中和恢复等待分开展示。

每个任务记录返回 `reviewToken`；云端读取最新 `runner.context` 后，在对应 plan action 中带上该 token。Node 校验当前任务版本、报告和检查证据。仅含验收/保留/优先级/延后等动作的验收批次，可应用仍有效的动作，只把发生变化的任务交给下一次复核；包含新增、重定向、取消等结构变化的计划继续保持原子性。普通并发结果更新会立即刷新快照，不增加模型失败轮次的等待，不反复重做整批验收。
