...
Cloud CHAT 使用同一个 `codex_cloud_collaboration action=completion.notify` 回传有界文本、固定 Node 本地文件槽或纯状态。Hub 落盘后通过 `session.callback.enqueue` 主动推给 Node；Node 立即唤醒空闲的 `callbackSessionId`，目标忙时在 Turn 结束事件后重试。Provider realtime、启动对账和低频定时查询只是漏通知恢复，而且未来新建的 CHAT/任务不保证已被外部定时器覆盖，因此不能替代这条主动回调。Hub/Node 内部仍保留 owner、generation、基线、持久队列、幂等 claim/ack、断线补漏和重启恢复，但这些不构成另一套公开协作模式。单主控、主控加协调者和单 AI 的传输链路相同，角色与进度由调用方管理；需要跨会话记忆时只用 `working_context` 保存一段短文本。
