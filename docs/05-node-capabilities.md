...
| `working.context` 2.0 | `get`, `set`, `clear` | 每个项目一段有界、带 revision 的普通文本，并附实时 Git facts |
...
| `agent.control` 1.6 | 分层 readiness、AI Harness/Route discovery、会话可见性双模式、Hub 主动 callback 投递与受控 Session/Turn 生命周期 | 当前 Harness 为本机 Codex + Claude Code |
...
`working.context` 只维护“一个项目一段文本”。`get` 读取当前文本、revision 与实时 Git branch/HEAD/dirty；`set` 写入最多 64 KiB 的 UTF-8 文本，可带 `expectedRevision` 做 CAS；`clear` 删除该项目的上下文。数据按规范化 `projectPath` 隔离并保存在 Node data-dir，不在项目中创建资料室目录或 Markdown 文件。

目标、进度、阻塞、决定和下一步如何排版由 AI 自己决定。简单任务不必写上下文；多 AI 协作时由主控合并一段短文本，禁止保存凭据、完整 Prompt、聊天全文、源码全文或长日志。旧 schema 1/2 状态读取时会尽量转换为普通文本，但旧 `plan.*`、`task.update`、`markdown.*`、`progress.watch` action 已移除。
...
`provider.readiness` 以 passive/safe 两种模式分别报告 `routeAvailable/providerAvailable/harnessAvailable/sessionBackendAvailable/readyForSessionCreate`，并为每层返回稳定 reasonCode 与耗时；safe 不创建 Session、不发送 Prompt。本地 Codex backend 使用只读 thread/list 检查 session backend；`chatgpt_cloud` 创建不依赖本地 thread/list，因此跳过该探测，只检查路由、同一个 Codex app-server 与 ChatGPT 登录，避免分派前无关的十几秒超时。ChatGPT 登录 token 直接由 Node 唯一 app-server 的 `getAuthStatus(includeToken=true)` 读取；readiness 会把 RPC 超时、明确未登录与其它 RPC 失败分别报告为 `CHATGPT_CLOUD_AUTH_RPC_TIMEOUT`、`CHATGPT_CLOUD_NOT_AUTHENTICATED` 与 `CHATGPT_CLOUD_AUTH_RPC_FAILED`，避免把超时误判成登录缺失。一次成功 readiness 取得的 token 会在内存中短时复用，紧随其后的 dispatch 不再重复调用鉴权 RPC；失败结果不缓存。对 `codex + chatgpt_cloud`，同一 app-server 代次内最近 30 秒的成功 safe 结果还可被紧随其后的创建预检复用；进程代次变化、隔离或超时后不会复用旧结果，失败时顶层 reasonCode 直接保留真实失败层。`session.create` 是 ChatGPT Cloud 的单一创建入口，`mode=complete|quick_chat` 选择等待首个回答或创建后快速返回；公网 MCP 必须携带 12-128 字符幂等键。Node 将包含 mode/model/thinking 的 spec hash 与小型结果持久化到 data-dir，重启后仍能重放同结果，key/spec 冲突或中间态不确定时拒绝重复创建。`models.list` 返回 `defaultModel`、两个 `creationModes`、实时 `modelPresets/thinkingOptions` 与 Node 本机配置的 `advancedModels`；Preset 与 Advanced 都可搭配 quick_chat/complete。Advanced 列表保存在 Node data-dir 的 `chatgpt-advanced-models.json`，不写死在源码或同步 Hub；Auto 不发送独立 thinking，其余预设档位作为 `thinking_effort` 发送；Advanced 模型还可声明自定义 thinking 值，并由调用方按模型支持情况选择。ChatGPT Cloud 默认 `mode=complete`，等待完整 SSE；`mode=quick_chat` 跳过 `/f/conversation/prepare`，默认使用 `model=auto`，收到真实 conversation ID 后立即返回 `phase=running/completionPending=true` 并在后台排空流。完整模式一旦 SSE 已返回 conversation ID，即使后续流结束异常也保存该 ID 并把执行状态标为 unknown，而不是把“已创建”重新解释成“未创建”。Provider 已明确拒绝且确认没有副作用的 create 会立即释放 reservation；真正不确定且没有已知 Session 的记录，须先用显式 `backend=chatgpt_cloud` 的 `session.list` 对账，再以 `session.delete + idempotencyKey + decision=confirm_not_created` 显式释放。

`session.callback.register/arm/enqueue/unregister/list/claim/ack` 是 `codex_cloud_collaboration` 的内部可靠性协议。公开 AI 不手工组装 callback route；统一调用 `action=dispatch`，由 Hub 创建或复用一个可见 Cloud CHAT、发送任务并把结果回调到 `callbackSessionId`。

Cloud CHAT 使用 `task_result_submit(taskRef,status,text?)` 提交有界文本、固定 Node 本地文件或纯状态。文件任务先用 `file_edit` 写完分配的 `resultPath`，再省略正文提交；Hub 只请求文件元数据和 SHA-256，不上传报告正文。旧 `codex_cloud_collaboration action=completion.notify` 保留兼容。Hub 落盘后通过 `session.callback.enqueue` 主动推给 Node；Node 唤醒空闲的 `callbackSessionId`，目标忙时等待。Provider realtime、启动对账和低频定时查询只是漏通知恢复，不能替代主动提交。Hub/Node 保留 owner、generation、持久队列、幂等 claim/ack 和重启恢复；角色与进度由调用方管理，需要跨会话记忆时使用 `working_context`。
...
Codex Adapter 只使用 Node 长期持有的一个 `codex app-server --stdio`。Provider 凭据与 ChatGPT/Codex 本地认证只留在 Node 本机。Codex 的 Harness model catalog 与 CC Switch upstream route 分开返回，不把两层模型名称强行合并。

`session.create` / `session.send` 使用 Codex 原生 Turn `UserInput`：text、skill、image、localImage、mention。创建走 `thread/start`，有首条输入时紧接 `turn/start`；续发未加载任务时先 `thread/resume`，再 `turn/start`。active 状态返回 `AGENT_SESSION_BUSY`，callback 继续留在 Node 队列等待重试，不会插入当前 Turn。`session.steer` 使用同一输入集合，但只追加到调用方明确给出的 active `turnId`，不隐式修改 model/cwd/outputSchema。Skill 输入使用 `name + absolute path`，不是把 Skill 内容拼接进 prompt。图片可统一指定 `imageDetail=auto|low|high|original`。`outputSchema` 作为有界 JSON Schema 直接传给 Turn，用于约束该 Turn 最终 assistant message；`session.result` 仍保留真实 raw `finalAgentMessage`，不伪装为已完成 typed decode。
...
- Codex app-server 进程重启后，Adapter 的 loaded generation 失效，并在下一次 Turn/Review 前自动 `thread/resume(threadId)`。
