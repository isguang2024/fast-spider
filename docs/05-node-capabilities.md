...
| `agent.control` 1.4 | 分层 readiness、AI Harness/Route discovery、会话可见性双模式、两阶段 callback 与受控 Session/Turn 生命周期 | 当前 Harness 为本机 Codex + Claude Code |
...
Cloud CHAT 的主完成路径仍是：先写固定本地结果，再通过 FastSpider_FS 调用 `codex_cloud_completion(action=notify)`；调度端批量 claim、验收并 ack。Node callback inbox 只保存无正文通知，是断线、漏通知或 CHAT 无法主动回调时的恢复兜底。Provider realtime 事件优先使用真实 event/turn/message ID 去重；复用 CHAT 的终态事件还会读取当前 identity 与持久基线核对，避免订阅重放旧完成回合。每个事件同时包含持久递增的本地 `event_sequence`。同一协调会话的多个 CHAT 通知保存在同一队列；`session.callback.list` 返回有界列表和固定 queue text，`session.callback.claim` 一次最多领取 64 条，领取租约为 5 分钟，完成 Hub 侧 notify/验收/ack 后再用 `session.callback.ack` 确认 Node 队列。领取租约超时会自动释放；重复 claim/ack 按持久状态对账。
