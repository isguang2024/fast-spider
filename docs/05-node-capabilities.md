...
| `agent.control` 1.5 | 分层 readiness、AI Harness/Route discovery、会话可见性双模式、类型化两阶段 callback 与受控 Session/Turn 生命周期 | 当前 Harness 为本机 Codex + Claude Code |
...
`session.callback.register/arm` 是 `codex_cloud_collaboration` 的内部路由协议，不是 AI 可单独拼装的任务入口；公网 `ai_control` 会以 `CALLBACK_ROUTE_MANAGED_ONLY` 拒绝直接创建或激活裸回调，避免 mission/task 在 Hub 不存在的孤儿路由。对复用的 CHAT，Hub 先从发送前的 `session.get` 提取当前 assistant/message identity，持久注册为未激活路由，再用任务代次稳定的 `idempotencyKey` 派生固定 Provider message ID 发送新任务，最后调用 `session.callback.arm`；激活后的首次补漏只有看到不同于基线的新 assistant identity 才能入队。新建 CHAT 也经过显式 arm。基线、armed 状态、generation 与 pending inbox 统一持久化在 Node data-dir 的 `agent/session-callbacks.json`；Hub 同时持久化投递阶段，进程或整机在注册、发送、激活任一步重启后会用同一 message ID 对账或续发，再恢复 arm，不会遗漏任务或重复新回合。旧 schema-1 注册升级时按原语义视为已激活。每个 source Cloud CHAT 只有一个 callback owner；generation 对应一次真实任务尝试，升级会清除旧代 pending，同一 generation 只接受一个终态通知。

Cloud CHAT 的主完成路径使用任务预登记的 `callbackType`：`local_file` 先通过 FastSpider_FS 文件能力把完整结果直接写到 Node 本地固定 `resultPath`，回调只引用该登记路径，禁止再次上传文件或由 CHAT 临时指定其它路径；`text` 直接回传短文本，最多 2000 个 Unicode 字符且最多 8192 个 UTF-8 字节；`status` 只回传 `completed|blocked|failed`，不带正文或路径。调度端批量 claim、按类型验收并 ack；一次最多领取 64 条，同时内联文本总量不超过 64 KiB。Node callback inbox 是断线、漏通知或 CHAT 无法主动回调时的恢复兜底：仅 `text` 保存同样受限的短文本，`local_file` 只保存路径引用，绝不复制或上传文件。Provider realtime 事件优先使用真实 event/turn/message ID 去重；复用 CHAT 的终态事件还会读取当前 identity 与持久基线核对，避免订阅重放旧完成回合。每个事件同时包含持久递增的本地 `event_sequence`。领取租约为 5 分钟，完成 Hub 侧 notify/验收/ack 后再用 `session.callback.ack` 确认 Node 队列；租约超时会自动释放，重复 claim/ack 按持久状态对账。

实时 callback 到达时只入队并立即发出本地唤醒；新增或释放 CHAT 会在同一条账号级官方 WebSocket 上动态订阅或退订，不等待重连。Node 不再固定每 30 秒扫描本地队列，而是在新事件、目标 Codex Turn 结束、claim/ack 变化或精确到期时间到达时才处理，用于释放过期领取和发送轻量通知，不读取 Cloud CHAT 状态。pending 最早存在约 5 分钟且目标会话空闲时才发第一次 nudge，此后同一目标最多约每 10 分钟再通知一次，真实投递或持久化错误才按约 30 秒重试，nudge 不携带任务结果正文。对 Provider 的 Cloud CHAT 状态查询以约 30 分钟为最低频率，只在启动后的首次恢复、官方长连接发生过中断或当前仍离线时执行；长连接持续健康时不会周期性重读全部 CHAT。`codex_cloud_collaboration` 默认每 30 分钟做一次持久自检，每回合只读取一次状态、执行至多一个有界动作并立即空闲；`tick` 无动作时返回 `idle=true + nextCheckAt`，过早的 `status.poll` 返回 `not_due + nextPollAt` 且不调用 Node/Provider；对疑似卡住的 CHAT 仅在到期且必要时才真正 `status.poll`；确认仍在运行时由 `chat.continue` 发送固定“请继续”，在观察到新进展前不重复发送；出现新进展会清零恢复计数，仍无进展时交给调度/主控决定是否人工接手或换代，不自动创建替代 CHAT。回调、状态检查或继续操作暴露的问题应通过 `working_context markdown.append` 以文件 revision CAS 追加到 `docs/progress/04-open-issues.md`；若读取返回 `NOT_FOUND`，先调用 `plan.init` 且设置 `initializeMarkdown=true` 初始化工作区，再读取并追加。不得写入密钥、原始 Provider payload、完整聊天记录或长日志。
