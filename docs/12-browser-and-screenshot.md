...
- `browser.automation` 1.3 固定动作：`readiness/launch/close/page.open/page.navigate/page.close/pages.list/click/type/press/wait/batch/snapshot/screenshot/events`。
...
MVP 选择 Node 管理的隔离 Profile：专用数据目录、无用户 Cookie/历史，生命周期、下载、网络和 Job 均由 Node 控制。截图临时文件只在 Node 本地存在到上传完成；未来若支持连接现有真实浏览器，必须作为独立高风险能力，要求用户显式启用并限定浏览器实例/Profile/域名范围。
...
Fast Spider 创建 ChatGPT 云端会话不使用浏览器：`ai_control` 的 `backend=chatgpt_cloud` 直接用 Codex app-server 的 ChatGPT 登录态 + 自解 Sentinel 走官方 `/backend-api/f/conversation`（见 docs/11 §4），会话出现在账号的 ChatGPT 聊天列表。
