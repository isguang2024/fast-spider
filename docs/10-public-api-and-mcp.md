...
| 创建或续发 Cloud CHAT，结果稍后回到 Codex | `codex_cloud_collaboration` | 即使只有一个简单任务也走持久中控；`task.dispatch` 成功后立即结束本轮，等待回调唤醒 |
