...
`codex_cloud_collaboration`（“Codex 云端协作”）是显式启用的 Hub 原生持久协作面，与普通 Chat 直接使用 FS 文件、Shell、Git、浏览器或 `ai_control` 完全分开；不需要 CHAT 协助时不应创建 collaboration。新建协作仍验证两个不同的本地 Codex 主控/调度会话，并确认同一 Codex app-server 的 ChatGPT 登录可用，本地会话验证只读取元数据。任务指定 `targetSessionId` 时，`task.dispatch` 只验证并续发该普通可见 ChatGPT Cloud CHAT，不关心它由哪个旧 Codex 创建；先取得精确 callback owner，再以 `mode=quick_chat` 投递。目标正在运行时返回 `AGENT_SESSION_BUSY`，不会扫描历史或创建替代会话。任务未指定 `targetSessionId` 时，分派直接新建隔离的 `backend=chatgpt_cloud + visibility=visible + mode=quick_chat` 会话，禁止通过 `session.list` 猜测旧 CHAT。`targetSessionId` 只是本任务代次的临时租约：复用 CHAT 在换代/关闭时精确注销 callback 并释放，不自动归档或删除；本次 collaboration 自己新建的 CHAT 才按原策略归档，删除仍必须有绑定精确任务的批准决定。每个任务都有固定本地 `resultPath`；CHAT 先写完整结果，再调用无正文的 `codex_cloud_completion notify`。Dispatcher 批量 claim、验收并 ack；Hub 队列、任务状态和 callback owner 在进程或整机重启后仍可恢复。对疑似卡住的 CHAT，调度先 `status.poll`，仍在运行才发送固定“请继续”，观察到新进展前不重复；是否人工接手或换代由主控明确决定。发现问题时使用 `working_context markdown.append` 以 CAS 写入 `docs/progress/04-open-issues.md`，`NOT_FOUND` 时先 `plan.init(initializeMarkdown=true)`；不得保存密钥、原始 payload 或完整聊天记录。
