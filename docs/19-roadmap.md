...
0.3.0 已完成权限模型收敛：Machine 是唯一远程边界；Node 直接以运行它的 OS 用户权限操作整台电脑。所有本机资源请求使用绝对路径，不再有目录授权注册、目录 ID、逐目录权限或目录列表工具。
...
screenshot_take, ai_control, artifact_get
```
...
### Phase 6：Local Bridge 与 Codex

- 当前用户 AF_UNIX/UDS Local Bridge 默认启用，可整体关闭。
- 本地和远程入口共用 Machine、OS 用户、Job、Event 和 Audit 事实。
- Codex 通过 `codex app-server --stdio` 实现 `bridge_owned` Session/Turn。
- `session.create` 使用绝对 `workingDirectory`；Provider Token 留在 Node 本机。
...
3. 保持 15 个 MCP 工具 Schema、错误码、Job/Event 和 Artifact 语义一致。
4. 完善 Browser 私网访问、下载清理、截图兼容和 Sidecar 版本矩阵。
5. 提高 Codex Session/Turn 的取消、重连、归档和模型发现稳定性。
...
- `desktop_owned`、Hook、handoff/recover 和通用 AI→AI 递归。
