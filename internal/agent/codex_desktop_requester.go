...
func (a *CodexAdapter) startDesktopOwnedTurn(ctx context.Context, sessionID string, inputs []map[string]any, options codexTurnOptions) (map[string]any, error) {
	...
	unlock := a.lockDesktopTurn(sessionID)
	defer unlock()
	dial := dialCodexDesktopIPC
	a.mu.Lock()
	if a.desktopRequestDial != nil {
		dial = a.desktopRequestDial
	}
	a.mu.Unlock()
	conn, err := dial()
	if err != nil {
	...
	if value := strings.TrimSpace(options.WorkingDirectory); value != "" {
		request["cwd"] = value
	}
	if value := strings.TrimSpace(options.Model); value != "" {
		request["model"] = value
	}
	if value := strings.TrimSpace(options.Effort); value != "" {
		request["effort"] = value
	}
	if value := strings.TrimSpace(options.Summary); value != "" {
		request["summary"] = value
	}
	if value := strings.TrimSpace(options.Personality); value != "" {
		request["personality"] = value
	}
	if value := strings.TrimSpace(options.ServiceTier); value != "" {
		request["serviceTier"] = value
	}
	if len(options.OutputSchema) > 0 {
		request["outputSchema"] = options.OutputSchema
	}
...
func (a *CodexAdapter) lockDesktopTurn(sessionID string) func() {
	a.desktopTurnLocksMu.Lock()
	if a.desktopTurnLocks == nil {
		a.desktopTurnLocks = make(map[string]*codexSessionLock)
	}
	lock := a.desktopTurnLocks[sessionID]
	if lock == nil {
		lock = &codexSessionLock{}
		a.desktopTurnLocks[sessionID] = lock
	}
	lock.refs++
	a.desktopTurnLocksMu.Unlock()
	...
	return func() {
		...
		lock.refs--
		if lock.refs == 0 {
			delete(a.desktopTurnLocks, sessionID)
		}
		a.desktopTurnLocksMu.Unlock()
	}
}
