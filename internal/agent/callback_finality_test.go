...
func TestLocalCallbackNudgeUsesLocalControlPlane(t *testing.T) {
	...
	if !strings.Contains(prompt, "FastSpider_Local local_capability") || !strings.Contains(prompt, "session.callback.claim") || !strings.Contains(prompt, "session.callback.ack") || strings.Contains(prompt, "FastSpider_FS") {
		t.Fatalf("local callback must stay on the local control plane: %s", prompt)
	}
	if !strings.Contains(prompt, "resume the current authorized task") || !strings.Contains(prompt, "Empty or already-consumed") {
...
func TestLocalCallbackNudgeJSONDrivesClaimAndAck(t *testing.T) {
	...
	parts := strings.Split(prompt, "FastSpider_Local local_capability(")
	if len(parts) != 3 {
		t.Fatalf("expected claim and ack calls, prompt=%q", prompt)
	}
	manager := New(t.TempDir(), nil)
	defer manager.Close(context.Background())
	manager.callbackStore = s
	manager.callbackDispatcher = nil
	for i, raw := range parts[1:] {
		...
		if call.Capability != "agent.control" {
			t.Fatalf("unexpected capability: %s", call.Capability)
		}
		if call.Action != []string{"session.callback.claim", "session.callback.ack"}[i] {
		...
		if _, err := manager.Control(context.Background(), call.Action, call.Params); err != nil {
			t.Fatalf("callback request through real Agent Control failed: %v", err)
		}
	}
