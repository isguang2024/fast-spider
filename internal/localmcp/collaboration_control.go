...
type collaborationControlInput struct {
	Action string         `json:"action" jsonschema:"collaboration.control v3: init/upgrade, tree/tree_update, apply, dispatch/dispatch_recover, inbox, resolve/decision_batch, analysis_prepare/result_recover, validation_claim/validation_receipt, retry, next_actions/record_check, archive/close. decision_batch accepts expectedRevision and 1..20 decisions: resultId,decision,evidenceRef plus validation/integration for accept, validationOwner for verify, blocker for block; accept may include followup=prepare to persist a delivery preparation task. analysis_prepare accepts expectedRevision,sourceItemIds,reason,question,brief and needs controller-approved mission.analysis_policy; delivery cannot choose model, scope or callback. result_recover is controller-only and accepts expectedRevision,resultId to verify the original provider manifest after CALLBACK_TEXT_TOO_LARGE, preserving existing business decisions. Inbox resultFetch gives exact full-result read parameters. The controller retains final decisions. Legacy actions remain available."`
	Params map[string]any `json:"params,omitempty" jsonschema:"structured action parameters, not a file path. next_actions returns pagination-safe refillInputs, bounded READY diagnostics, validation binding calls, executionCheck and recordCheck parameters. record_check requires dbPath,missionId,actorSessionId,expectedRevision,expectedObservationRevision,actionId,outcome,evidenceRef; optional retryAt; no itemId or evidence. get/brief/tree are ledger-only, not execution checks. Local execution_ref: codex-thread:<threadId> or codex-agent:<parentThreadId>#<canonicalPath>; local_scope: {machineId,workingDirectory,accessMode:read_only|write,writeScope?:string[]}. Cloud packet.writeScope is one string. Never Cloud credentials."`
}
