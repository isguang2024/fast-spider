...
type collaborationControlInput struct {
	Action string         `json:"action" jsonschema:"collaboration.control v3: init, upgrade, tree/tree_update, apply, dispatch/dispatch_recover, inbox, resolve, decision_batch, validation_claim/validation_receipt, retry, next_actions/record_check, archive, close. Controller apply can bind mission.delivery_coordinator with coordination_ref while paused. decision_batch takes expectedRevision and 1..20 frozen decisions and commits atomically before ACK. Legacy actions remain available."`
