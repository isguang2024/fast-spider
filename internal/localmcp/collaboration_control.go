...
type collaborationControlInput struct {
	Action string         `json:"action" jsonschema:"collaboration.control v3: init/upgrade, tree/tree_update, apply, dispatch/dispatch_recover, inbox, resolve/decision_batch, analysis_prepare, validation_claim/validation_receipt, retry, next_actions/record_check, archive/close. decision_batch accepts expectedRevision and 1..20 decisions: resultId,decision,evidenceRef plus validation/integration for accept, validationOwner for verify, blocker for block; accept may include followup=prepare to persist a delivery preparation task. analysis_prepare accepts expectedRevision,sourceItemIds,reason,question,brief and needs controller-approved mission.analysis_policy; delivery cannot choose model, scope or callback. The controller retains final decisions. Legacy actions remain available."`
