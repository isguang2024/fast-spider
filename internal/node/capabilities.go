...
func (c *Client) handleCapabilityRequestFrom(ctx context.Context, req protocolv1.CapabilityRequest, local bool) protocolv1.CapabilityResponse {
	...
	if req.Capability == "agent.control" && strings.HasPrefix(req.Action, "runner.") && req.Action != "runner.submit" && !local {
		response.Error = protocolError("UNSUPPORTED_CAPABILITY", "native runner control actions are local-only; Cloud CHAT may only submit its bound result", false)
		return response
	}
