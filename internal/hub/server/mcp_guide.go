...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		Description: "Direct Codex/Claude/ChatGPT cloud CHAT session control. Windows desktopBridge uses the Desktop owner/control bridge; nativeConversationStreaming=unsupported. For async callbacks use codex_cloud_collaboration, then yield after dispatch instead of polling.",
		WhenToUse:   []string{"Discover AI runtimes", "Direct interactive or lifecycle control of Codex/Claude/ChatGPT Cloud sessions", "Recover a collaboration callback through the Node queue"}, RequiredInputs: []string{"machineId and action", "workingDirectory for scoped list/create", "session.result manifest: resultMode=manifest when no deliverable path is assigned", "callback fallback consumption: callbackTargetSessionId and optional callbackClaimLimit"},
		SafeSequence: []string{
			"machine_list",
			"Choose the entry first: direct conversation or lifecycle work uses ai_control; create-or-continue CHAT work that must return later by callback uses codex_cloud_collaboration, even for one simple task",
			...
			"Bare ai_control quick_chat has no collaboration callback contract, so session.get/watch/result is only for an explicitly interactive caller; never use that polling loop to emulate callback-based delegation",
		...
		RecommendedNext: []string{"capability_list(view=workflow,name=cloud-chat-callback) for asynchronous delegation", "session.get/watch/result only for direct interactive sessions", "session.callback.list only for fallback recovery"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "UNSUPPORTED_SESSION_PLUGIN_BINDING", "CALLBACK_ROUTE_MANAGED_ONLY", "CALLBACK_OWNER_CONFLICT", "CALLBACK_GENERATION_STALE", "AGENT_CALLBACK_STORE_UNAVAILABLE"}, BoundedExamples: []map[string]any{{"machineId": "<machine-id>", "action": "session.list", "workingDirectory": "<absolute-project>"}, {"machineId": "<machine-id>", "action": "session.callback.list", "providerId": "codex", "backend": "chatgpt_cloud", "callbackTargetSessionId": "<dispatcher-session>"}},
	},
	"codex_cloud_collaboration": {
		Description: "Codex 云端协作：一个简单任务或长期任务只要需要创建/续发可见 ChatGPT Cloud CHAT 并稍后回调，都使用此持久中控入口；成功 dispatch 后当前调度立即结束本轮，不主动等待或轮询。",
		WhenToUse:   []string{"One simple Cloud CHAT task that should return later by callback", "Unattended multi-step work", "Local Codex coordinates existing Cloud CHAT sessions", "Cloud CHAT self-callback through FastSpider_FS", "Recovery across context limits or missed callbacks"}, RequiredInputs: []string{"Read capability_list(view=workflow,name=cloud-chat-callback) for the one-task path or codex-cloud-collaboration for the full path", "simple mode sets controllerSessionId and dispatcherSessionId to the same existing local Codex callback target; multi-role mode supplies two different existing local Codex sessions", "create also requires machineId, goal/doneWhen, workingDirectory, limits, allowedActions and idempotencyKey", "task.add selects callbackType=local_file|text|status; local_file is the backward-compatible default", "Cloud CHAT completion uses codex_cloud_collaboration action=completion.notify with actorSessionId=$self, taskId, outcome, callbackType, and text only for text callbacks", "collaboration mutations require revision and lease, but completion notify/claim/ack do not"},
		SafeSequence: []string{
			"create performs local Codex + ChatGPT login readiness checks",
			"for one Codex plus one CHAT, set controllerSessionId=dispatcherSessionId to the existing local Codex callback target; no extra controller or dispatcher AI is created",
			...
			"after a successful task.dispatch, obey callerShouldYield=true and nextAction=end_turn: return only a short dispatch receipt and end the current turn; do not call session.get, session.watch, session.result, tick, or status.poll while waiting",
		...
		}, Returns: []string{"Controller-safe decision brief or dispatcher execution view", "task.dispatch receipt with awaitMode=callback, activePollingAllowed=false, callerShouldYield and nextAction=end_turn", "persistent revision and collaboration status"}, RecommendedNext: []string{"end the current turn after successful task.dispatch", "on callback wake use completion.claim/completion.ack", "capability_list(view=workflow,name=codex-cloud-collaboration)"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "CONFLICT", "UNAUTHORIZED", "RESOURCE_LIMIT", "EXPIRED", "CALLBACK_BASELINE_UNAVAILABLE", "ORPHAN_CALLBACK_ROUTE"},
...
var mcpToolSummaryDefinitions = []mcpToolSummary{
	...
	{Name: "codex_cloud_collaboration", Category: "codex-cloud-collaboration", Summary: "Create or continue a callback-based CHAT task, then yield immediately until the durable wake arrives.", Guide: "capability_list(view=tool,name=codex_cloud_collaboration)"},
...
func codexCloudCollaborationGuide() (string, []string, []string, []string) {
	summary := "Use the persistent controller path whenever Codex creates or continues a visible ChatGPT Cloud CHAT and expects a later callback, including one simple task. After task.dispatch succeeds, end the current turn and let callback or scheduled recovery wake the dispatcher; never stay active to poll."
	whenToUse := []string{"One simple Cloud CHAT task should return later by callback", "A current Codex needs optional Cloud CHAT assistance", "The user explicitly supplies a CHAT ID to continue", "A new unrelated delegated task needs a clean CHAT", "Recovery across restarts, context limits, or missed callbacks"}
	requiredInputs := []string{
		"Read capability_list(view=workflow,name=cloud-chat-callback) for a simple one-task delegation or codex-cloud-collaboration for the full control flow",
		"simple mode uses one existing local Codex callback target as both controllerSessionId and dispatcherSessionId; multi-role mode uses two different existing local Codex sessions",
		"create also requires machineId, goal/doneWhen, workingDirectory, limits, allowedActions and idempotencyKey",
	...
	safeSequence := []string{
		...
		"Choose by delivery semantics, not task size: if Codex must receive a later result, use this controller path; use bare ai_control only when the caller intentionally owns an interactive wait/read loop",
		"create performs local Codex plus ChatGPT login readiness checks using metadata-only local session validation",
		"For one Codex plus one CHAT, set controllerSessionId=dispatcherSessionId to that existing local Codex task ID; Fast Spider validates it once, registers immediate durable wake delivery, and creates no extra controller or dispatcher AI",
		...
		"A successful task.dispatch returns awaitMode=callback, activePollingAllowed=false, callerShouldYield=true and nextAction=end_turn; immediately return a short dispatch receipt and end the current turn without session.get/watch/result, tick, status.poll, sleep, or repeated reasoning",
...
var mcpWorkflowGuides = map[string]mcpWorkflowGuideEntry{
	...
	"cloud-chat-callback":       {Summary: "Dispatch one simple visible Cloud CHAT task to a specified Codex callback target and stop the current turn until the durable wake arrives.", RequiredInputs: []string{"the current or another existing local Codex session ID as callback target", "machineId and normal collaboration create inputs", "one task with prompt, allowedActions and callbackType", "optional exact targetSessionId for the CHAT"}, SafeSequence: []string{"Use codex_cloud_collaboration rather than bare ai_control because the result must return later by callback", "create with controllerSessionId=dispatcherSessionId=<callback Codex session ID>; this single-session mode does not create a separate controller or dispatcher AI", "lease.acquire using that same Codex session ID", "task.add with callbackType=local_file for a document, text for at most 2000 Unicode characters and 8192 UTF-8 bytes, or status for no payload", "task.dispatch creates or continues exactly one visible quick_chat and registers immediate event-driven wake delivery to the specified Codex ID", "when task.dispatch returns callerShouldYield=true and nextAction=end_turn, return a short receipt and end the current turn immediately", "do not call session.get, session.watch, session.result, tick, status.poll, sleep, or keep reasoning while waiting", "on wake, batch completion.claim, verify by type, completion.ack; Node keeps a persistent fallback queue and reminder if direct delivery finds the target busy"}, Returns: []string{"asynchronous dispatch receipt", "direct durable wake to the specified Codex session", "restart-safe Hub and Node recovery state"}, RecommendedNext: []string{"end the current turn after dispatch", "completion.claim/completion.ack after wake"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "AGENT_SESSION_BUSY", "CALLBACK_BASELINE_UNAVAILABLE", "ORPHAN_CALLBACK_ROUTE"}},
	...
	"codex-cloud-collaboration": {Summary: "Run explicit persistent collaboration across existing normal visible ChatGPT Cloud CHAT sessions through local Codex; the current Cloud CHAT performs the primary FastSpider_FS callback and Node delivery is fallback recovery.", RequiredInputs: []string{"machineId and absolute workingDirectory", "one existing local Codex callback target used as both controller and dispatcher, or two different sessions for multi-role orchestration", "goal, doneWhen, allowedActions, limits and stable idempotencyKey"}, SafeSequence: []string{"codex_cloud_collaboration(create) verifies local Codex and ChatGPT authentication", "dispatcher lease.acquire", "goal.add", "task.add with inherited permissions, non-overlapping write scope, callbackType=local_file|text|status and optional local_file deliverablePath", "task.dispatch creates exactly one backend=chatgpt_cloud visibility=visible conversation and forbids new Cloud Worker/CHAT creation", "Cloud CHAT calls action=get with actorSessionId=$self and taskId to obtain callbackType and any fixed local resultPath", "Cloud CHAT follows the selected type: local_file writes the registered Node-local path without upload, text stays bounded, status carries no payload, then it calls codex_cloud_completion notify", "dispatcher batch claims, verifies, and acknowledges completion; archive is asynchronous", "Node callback/session polling and tick/status.poll are fallback recovery only", "for a stalled CHAT call status.poll, then chat.continue sends 请继续 if it is still running and suppresses duplicates until progress; if it remains stalled, surface chat_recovery_decision and never create a replacement automatically", "on NOT_FOUND call working_context plan.init with initializeMarkdown=true, then read and CAS-append the bounded issue to docs/progress/04-open-issues.md", "compact and chat.rotate only after an explicit replacement decision", "close archives by default"}, Returns: []string{"Persistent collaboration revision, role-filtered state and bounded next actions"}, RecommendedNext: []string{"end the current turn after dispatch", "completion.claim/completion.ack after wake"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "CONFLICT", "UNAUTHORIZED", "RESOURCE_LIMIT", "EXPIRED"}},
}
