...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		Description: "Control Codex/Claude Code and ChatGPT cloud CHAT sessions. On Windows, desktopBridge uses the Desktop owner/control bridge; nativeConversationStreaming=unsupported. Cloud callbacks batch fixed envelopes into one idle local coordinator.",
		WhenToUse:   []string{"Discover AI runtimes", "Control Codex/Claude/ChatGPT Cloud sessions", "Register callback-first unattended coordination"}, RequiredInputs: []string{"machineId and action", "workingDirectory for scoped list/create", "callback register: backend=chatgpt_cloud, source sessionId, callbackTargetSessionId, callbackMissionId, callbackTaskId and positive callbackGeneration"},
		SafeSequence: []string{"machine_list", "provider.readiness and models.list before Cloud create; localCreateDefaults are preferences across preset and advanced choices", "ChatGPT cloud CHAT session.create: providerId=codex, backend=chatgpt_cloud, visibility=visible, mode=quick_chat|complete; returns externalIdType=chatgpt_conversation; quick_chat completionPending=true; reuse the original idempotencyKey after timeout or disconnect", "Do not pass pluginName to chatgpt_cloud session.create: installed/catalog plugins do not create a per-session binding and return UNSUPPORTED_SESSION_PLUGIN_BINDING before reservation/provider work", "session.list incomplete=true is sidecar-only and never proves not-created", "register one callback destination per source Cloud CHAT; callbackDeliverablePath skips CHAT content reread and reports local file metadata", "completion is persisted before wake; concurrent events share one fixed envelope; active targets remain queued", "for persistent multi-CHAT work use codex_cloud_collaboration; ordinary ai_control remains a direct session API", "unregister with the current generation at close"}, Returns: []string{"bounded provider/session results", "models.list live choices plus localCreateDefaults", "callback registry and pending count", "callback-first delivery with heartbeat-fallback recovery"},
	...
	"codex_cloud_collaboration": {
		Description: "Codex 云端协作：persist bounded normal visible Cloud CHAT hierarchies through an available local Codex; never fall back to another AI provider or ordinary FS operations.",
		WhenToUse:   []string{"Unattended multi-step work", "Local Codex coordinates child Cloud CHAT sessions", "Recovery across context limits or missed callbacks"}, RequiredInputs: []string{"Read capability_list(view=workflow,name=codex-cloud-collaboration) first", "create requires two existing local Codex controller/dispatcher sessions plus machineId, goal/doneWhen, workingDirectory, limits, allowedActions and idempotencyKey", "mutations require actorSessionId/actorRole, expectedRevision and an active dispatcher lease"},
		SafeSequence: []string{"create performs local Codex + ChatGPT login readiness checks", "lease.acquire for the dispatcher", "goal.add and task.add; write tasks may assign deliverablePath inside writeScope", "task.dispatch", "callback event.ingest then event.ack with the verified SHA-256", "task.update marks the accepted result done", "tick and status.poll only as recovery", "close archives Cloud CHAT sessions by default"}, Returns: []string{"Controller-safe decision brief or dispatcher execution view", "bounded next actions", "persistent revision and collaboration status"}, RecommendedNext: []string{"codex_cloud_collaboration(action=tick)", "capability_list(view=workflow,name=codex-cloud-collaboration)"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "CONFLICT", "UNAUTHORIZED", "RESOURCE_LIMIT", "EXPIRED"},
...
var mcpToolSummaryDefinitions = []mcpToolSummary{
	...
	{Name: "codex_cloud_collaboration", Category: "codex-cloud-collaboration", Summary: "Coordinate ordinary visible Cloud CHAT hierarchies through an available local Codex only.", Guide: "capability_list(view=tool,name=codex_cloud_collaboration)"},
...
var mcpWorkflowGuides = map[string]mcpWorkflowGuideEntry{
	...
	"codex-cloud-collaboration": {Summary: "Run explicit persistent collaboration across normal visible ChatGPT Cloud CHAT sessions through local Codex only.", RequiredInputs: []string{"machineId and absolute workingDirectory", "two existing local Codex controller and dispatcher session IDs", "goal, doneWhen, allowedActions, limits and stable idempotencyKey"}, SafeSequence: []string{"codex_cloud_collaboration(create) verifies local Codex and ChatGPT authentication", "dispatcher lease.acquire", "goal.add", "task.add with inherited permissions, non-overlapping write scope and optional deliverablePath", "task.dispatch creates backend=chatgpt_cloud visibility=visible conversations", "callback event.ingest/ack binds the verified SHA-256", "task.update accepts result_available as done", "tick then status.poll only for missed notification or quiet recovery", "compact and chat.rotate before context replacement", "close archives by default"}, Returns: []string{"Persistent collaboration revision, role-filtered state and bounded next actions"}, RecommendedNext: []string{"codex_cloud_collaboration"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "CONFLICT", "UNAUTHORIZED", "RESOURCE_LIMIT", "EXPIRED"}},
}
...
func newMCPGuide(serverVersion, view, name string) (*mcpGuide, error) {
	...
	case "overview":
		...
		base.Categories = []mcpGuideCategory{
			...
			{Name: "Codex 云端协作", Summary: "Persistently coordinate ordinary visible ChatGPT Cloud CHAT sessions through an available local Codex only.", Tools: []string{"codex_cloud_collaboration"}},
