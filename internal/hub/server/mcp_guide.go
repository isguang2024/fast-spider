...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		...
		SafeSequence: []string{"machine_list", "providers.list or provider.readiness to inspect desktopBridge before relying on Desktop control", "provider.readiness(providerId=codex, backend=chatgpt_cloud, mode=safe) for cloud create", "models.list with backend=chatgpt_cloud, then select one returned modelPreset instead of independently mixing model and thinking", "session.create with backend=chatgpt_cloud; use mode=quick_chat for Codex-style fast return or mode=complete to await the first answer", "after create use sessionId directly for get/send/watch/result", "for unresolved cloud create reconciliation use session.list with backend=chatgpt_cloud", "session.steer only for an active cloud turn with asyncTaskId"}, Returns: []string{"bounded provider, route, desktopBridge or session results", "models.list defaultModel, creationModes and modelPresets", "session.create visibility metadata and provider-native external ID", "cloud sessionId plus externalIdType=chatgpt_conversation", "quick_chat returns phase=running and completionPending=true"},
