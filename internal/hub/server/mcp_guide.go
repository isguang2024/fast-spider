...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		Description: "Control Codex/Claude Code. Windows defaults to the Desktop owner/control bridge; desktopBridge reports state and nativeConversationStreaming=unsupported limits live history. ChatGPT cloud CHAT: session.create with providerId=codex, backend=chatgpt_cloud, visibility=visible; mode=quick_chat returns when the conversation ID arrives.",
		...
		SafeSequence: []string{"machine_list", "providers.list or provider.readiness to inspect desktopBridge before relying on Desktop control", "provider.readiness(providerId=codex, backend=chatgpt_cloud, mode=safe) for cloud create", "session.create with backend=chatgpt_cloud; use mode=quick_chat for Codex-style fast return or omit mode to await completion", "after create use sessionId directly for get/send/watch/result", "for unresolved cloud create reconciliation use session.list with backend=chatgpt_cloud", "session.steer only for an active cloud turn with asyncTaskId"}, Returns: []string{"bounded provider, route, desktopBridge or session results", "session.create visibility metadata and provider-native external ID", "cloud sessionId plus externalIdType=chatgpt_conversation", "quick_chat returns phase=running and completionPending=true"},
