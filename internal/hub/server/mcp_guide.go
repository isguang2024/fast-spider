...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"audit_log": {
		Description:    "Read recent Hub mutation audit entries for the current MCP owner. This is Hub-local and does not require a Node online; optional filters include machineId, actionPrefix, result, before and limit.",
	...
	"git_control": {
		Description: "Use for allowlisted Git reads and authorized writes/network actions. Requires machineId, absolute repositoryPath and action; returns Git facts or a Job/result. fetch/pull/push may omit remote: Node resolves current-branch upstream, then origin, then a sole configured remote; ambiguous remotes fail closed.",
	...
	"browser_control": {
		Description: "Use for isolated Chromium automation. Requires machineId and action-specific IDs; returns readiness/session/page/snapshot refs; screenshot actions return URL-only temporary attachment metadata rather than native chat images.",
	...
	"screenshot_take": {
		Description: "Use for one-time desktop/display/window visual evidence, not browser element targeting. Requires machineId and action; returns URL-only metadata for a temporary screenshot attachment instead of embedding the image in chat.",
	...
	"ai_control": {
		...
		SafeSequence: []string{"machine_list", "provider.readiness(providerId=codex, backend=chatgpt_cloud, mode=safe) for cloud create", "session.create with backend=chatgpt_cloud", "after create use sessionId directly for get/send/watch/result", "for unresolved cloud create reconciliation use session.list with backend=chatgpt_cloud", "session.steer only for an active cloud turn with asyncTaskId"}, Returns: []string{"bounded provider, route or session results", "session.create visibility metadata and provider-native external ID", "cloud sessionId plus externalIdType=chatgpt_conversation"},
	...
	"artifact_get": {
		Description: "Use to upload/read local files or terminal Job logs. uploadFile/uploadJobLog/get return bounded native MCP content when safe. publishFile is the explicit temporary attachment relay: it returns URL metadata only, never embeds the file in chat, and the URL expires after 48 hours.",
