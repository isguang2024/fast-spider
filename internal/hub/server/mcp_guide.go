...
var mcpWorkflowGuides = map[string]mcpWorkflowGuideEntry{
	...
	"cloud-chat-callback":       {Summary: "Dispatch one simple visible Cloud CHAT task through the persistent controller and stop the current turn until a callback wake arrives.", RequiredInputs: []string{"existing collaboration or the normal create inputs", "dispatcher lease", "one task with prompt, allowedActions and callbackType", "optional exact targetSessionId"}, SafeSequence: []string{"Use codex_cloud_collaboration rather than bare ai_control because the result must return later by callback", "lease.acquire when needed", "task.add with callbackType=local_file for a document, text for at most 2000 Unicode characters and 8192 UTF-8 bytes, or status for no payload", "task.dispatch creates or continues exactly one visible quick_chat and registers the durable callback", "when task.dispatch returns callerShouldYield=true and nextAction=end_turn, return a short receipt and end the current turn immediately", "do not call session.get, session.watch, session.result, tick, status.poll, sleep, or keep reasoning while waiting", "on callback or fallback wake, batch completion.claim, verify by type, completion.ack, then close if the one task is complete"}, Returns: []string{"asynchronous dispatch receipt", "durable typed callback after wake", "restart-safe Hub and Node recovery state"}, RecommendedNext: []string{"end the current turn after dispatch", "completion.claim/completion.ack after wake"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "AGENT_SESSION_BUSY", "CALLBACK_BASELINE_UNAVAILABLE", "ORPHAN_CALLBACK_ROUTE"}},
