...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		Description: "Control Codex/Claude Code and ChatGPT cloud CHAT sessions. On Windows, desktopBridge uses the Desktop owner/control bridge; nativeConversationStreaming=unsupported. For codex_cloud_collaboration, the existing Cloud CHAT must call FastSpider_FS event.ingest then event.ack; Node callback delivery is recovery fallback only.",
		...
		SafeSequence: []string{"machine_list", "provider.readiness and models.list before Cloud create; localCreateDefaults are preferences across preset and advanced choices", "ChatGPT cloud CHAT session.create: providerId=codex, backend=chatgpt_cloud, visibility=visible, mode=quick_chat|complete; returns externalIdType=chatgpt_conversation; quick_chat completionPending=true; reuse the original idempotencyKey after timeout or disconnect", "Do not pass pluginName to chatgpt_cloud session.create: installed/catalog plugins do not create a per-session binding and return UNSUPPORTED_SESSION_PLUGIN_BINDING before reservation/provider work", "session.list incomplete=true is sidecar-only and never proves not-created", "register one callback destination per source Cloud CHAT; callbackDeliverablePath skips CHAT content reread and reports local file metadata", "for codex_cloud_collaboration, the current Cloud CHAT calls codex_cloud_collaboration(action=get, actorSessionId=$self, actorRole=chat, taskId=...) before work, then calls event.ingest and event.ack before its final message", "use file_read statOnly=true for deliverable size/fileSha256, or ai_control session.result resultMode=manifest for Result Pool metadata when no deliverable exists", "Node callback delivery is recovery fallback only: session.callback.list exposes the queue, session.callback.claim takes up to 64 results with a five-minute lease, and session.callback.ack confirms the batch", "Node checks its local callback queue about every 30 seconds, waits about five minutes before the first lightweight nudge, repeats nudges no more often than about 10 minutes, and reads Cloud CHAT provider status about every 10 minutes only for missed completion recovery", "if callback recovery exposes a problem or question, read then CAS-append a bounded note through working_context to docs/progress/04-open-issues.md; never store secrets, raw provider payloads, full transcripts, or long logs", "ordinary ai_control remains a direct session API", "unregister with the current generation at close"}, Returns: []string{"bounded provider/session results", "models.list live choices plus localCreateDefaults", "callback registry, queue text and pending metadata", "Cloud CHAT primary callback plus Node queued-batch-claim fallback"},
...
func newMCPGuide(serverVersion, view, name string) (*mcpGuide, error) {
	...
	case "overview":
		...
		base.Categories = []mcpGuideCategory{
			...
			{Name: "Codex 云端协作", Summary: "Coordinate existing visible ChatGPT Cloud CHAT sessions; the current CHAT calls FS ingest/ack and Node is fallback recovery.", Tools: []string{"codex_cloud_collaboration"}},
