...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		Description: "Control Codex/Claude Code. Windows defaults to the Desktop owner/control bridge; desktopBridge reports state and nativeConversationStreaming=unsupported limits live history. ChatGPT cloud CHAT uses one session.create entry with providerId=codex, backend=chatgpt_cloud, visibility=visible and mode=quick_chat|complete; models.list separates preset/advanced configuration and returns model/thinking choices.",
		WhenToUse:   []string{"Discover AI runtimes", "List or control Codex/Claude Code sessions", "Create or exchange messages with a visible ChatGPT cloud CHAT conversation through Codex"}, RequiredInputs: []string{"machineId", "action", "providerId when not codex", "workingDirectory for scoped list/create", "ChatGPT cloud create: providerId=codex, backend=chatgpt_cloud, visibility=visible, prompt, absolute workingDirectory and idempotencyKey; optional mode=quick_chat|complete"},
		SafeSequence: []string{"machine_list", "providers.list or provider.readiness to inspect desktopBridge before relying on Desktop control", "provider.readiness(providerId=codex, backend=chatgpt_cloud, mode=safe) for cloud create", "models.list with backend=chatgpt_cloud; preset configuration selects one live modelPreset, advanced configuration selects a Node-configured advancedModels entry plus an optional live thinkingOption", "session.create with backend=chatgpt_cloud; either configuration can use mode=quick_chat for fast return or mode=complete to await the first answer", "after create use sessionId directly for get/send/watch/result; session.send inherits the first assistant turn model and thinking effort unless explicitly overridden", "for unresolved cloud create reconciliation use session.list with backend=chatgpt_cloud", "session.steer only for an active cloud turn with asyncTaskId"}, Returns: []string{"bounded provider, route, desktopBridge or session results", "models.list defaultModel, creationModes, configurationModes, modelPresets, models, advancedModels and live thinkingOptions", "session.create visibility metadata and provider-native external ID", "cloud sessionId plus externalIdType=chatgpt_conversation", "quick_chat returns phase=running and completionPending=true"},
		RecommendedNext: []string{"session.get", "session.watch", "session.result"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "AGENT_SESSION_VISIBILITY_UNSUPPORTED", "CHATGPT_CLOUD_NOT_AUTHENTICATED", "CHATGPT_CLOUD_UNAVAILABLE", "DEADLINE_EXCEEDED"}, BoundedExamples: []map[string]any{{"machineId": "<machine-id>", "action": "session.list", "workingDirectory": "<absolute-project>"}, {"machineId": "<machine-id>", "action": "session.create", "providerId": "codex", "backend": "chatgpt_cloud", "mode": "quick_chat", "visibility": "visible", "prompt": "<first-chat-message>", "workingDirectory": "<absolute-project>", "idempotencyKey": "chatgpt-cloud-001"}},
	},
	"working_context": {
