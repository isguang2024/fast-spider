...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		...
		SafeSequence: []string{
			...
			"for codex_cloud_collaboration, the current Cloud CHAT reads its fixed resultPath, writes the result, then calls codex_cloud_completion notify before its final message",
			"the dispatcher uses file_read statOnly=true or a compatible Result Pool manifest only after batch claim, then calls codex_cloud_completion ack",
		...
		}, Returns: []string{"bounded provider/session results", "models.list live choices plus localCreateDefaults", "callback registry, queue text and pending metadata", "Cloud CHAT primary callback plus Node queued-batch-claim fallback"},
	...
	"codex_cloud_collaboration": {
		...
		SafeSequence: []string{
			...
			"goal.add and task.add; write tasks may assign deliverablePath inside writeScope",
			...
			"the Cloud CHAT calls codex_cloud_collaboration(action=get, actorSessionId=$self, actorRole=chat, taskId=...) to obtain the current task and fixed resultPath",
			"write the complete result to the fixed local resultPath before notification; the dispatcher verifies it after claim",
			"the same Cloud CHAT calls codex_cloud_completion notify with no body or result metadata; the dispatcher batch claims, verifies, and acknowledges it",
	...
	"codex_cloud_completion": {
		...
		CommonErrors:    []string{"INVALID_REQUEST", "UNAUTHORIZED", "CONFLICT", "EXPIRED", "COLLABORATION_NOT_FOUND", "TASK_NOT_FOUND"},
