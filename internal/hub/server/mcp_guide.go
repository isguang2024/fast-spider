...
var mcpToolGuides = map[string]mcpToolGuideEntry{
	...
	"ai_control": {
		...
		SafeSequence: []string{"machine_list", "provider.readiness and models.list before Cloud create; localCreateDefaults are preferences across preset and advanced choices", "ChatGPT cloud CHAT session.create: providerId=codex, backend=chatgpt_cloud, visibility=visible, mode=quick_chat|complete; it returns externalIdType=chatgpt_conversation and quick_chat completionPending=true", "create or reconcile the ChatGPT Cloud session", "register one callback owner per source Worker", "completion is persisted before wake; concurrent events share one fixed envelope; active targets remain queued", "the coordinator reads session.result itself; heartbeat is only restart or missed-notification recovery", "unregister with the current generation at mission close"}, Returns: []string{"bounded provider/session results", "models.list live choices plus localCreateDefaults", "callback registry and pending count", "callback-first delivery with heartbeat-fallback recovery"},
		RecommendedNext: []string{"session.get", "session.watch", "session.result", "session.callback.list"}, CommonErrors: []string{"RUNTIME_UNAVAILABLE", "INVALID_REQUEST", "CALLBACK_OWNER_CONFLICT", "CALLBACK_GENERATION_STALE", "AGENT_CALLBACK_STORE_UNAVAILABLE"}, BoundedExamples: []map[string]any{{"machineId": "<machine-id>", "action": "session.list", "workingDirectory": "<absolute-project>"}, {"machineId": "<machine-id>", "action": "session.callback.register", "providerId": "codex", "backend": "chatgpt_cloud", "sessionId": "<cloud-worker-session>", "callbackTargetSessionId": "<local-coordinator-session>", "callbackMissionId": "mission-1", "callbackTaskId": "task-1", "callbackGeneration": 1}},
	},
	"working_context": {
