...
func TestAgentCapabilityAdvertisesCurrentActionContract(t *testing.T) {
	want := []string{
		...
		"runner.init", "runner.status", "runner.pause", "runner.resume", "runner.goal", "runner.signal", "runner.add", "runner.wake", "runner.submit", "runner.checkpoint", "runner.change", "runner.cancel", "runner.archive", "runner.unarchive", "runner.configure",
	}
	if !reflect.DeepEqual(AgentCapability.Actions, want) {
		t.Fatalf("agent.control actions=%v want=%v", AgentCapability.Actions, want)
	}
	if AgentCapability.Version != "1.9" {
		t.Fatalf("agent.control version=%q want 1.9", AgentCapability.Version)
	}
}
