...
func TestAgentCapabilityAdvertisesCurrentActionContract(t *testing.T) {
	want := []string{
		...
		"session.list", "session.get", "session.create", "session.send", "session.steer", "session.respond", "session.watch", "session.callback.register", "session.callback.unregister", "session.callback.list", "session.callback.claim", "session.callback.ack", "session.cancel", "session.result", "session.rename", "session.archive",
	...
	if AgentCapability.Version != "1.3" {
		t.Fatalf("agent.control version=%q want 1.3", AgentCapability.Version)
	}
}
